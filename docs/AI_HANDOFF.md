...
猛鬼宿舍·躺平发育 v0.5.0 已完成并推送 GitHub（xycjscs/menggui-tangping）。
**核心逻辑 196/196 测试通过、3 天数值模拟达标（1091波/0门破/109 Boss全杀）、Web 试玩版线上可玩、小程序三屏+道具接入完成。**
v0.2 新增：英雄系统(4位) / Boss波(每10波) / 每日任务(3个) / 成就(10个) / 3 个新广告位。
v0.3 新增：2D 可视化战场（俯视战棋，canvas 60fps，移动怪物，Web/小程序共用 battleView）。
v0.4 新增：三屏游戏流程（首页关卡选择/长效钱包/奖励 → 进入页关卡详情+支援 → 游戏屏战场为主+点触菜单）+ 关卡制（8 关，通关解锁+星级）。
**v0.5 新增：可放置道具系统（战场 4 插槽 × 4 类道具：路障吸伤/尖刺加DPS/招财灯产币/急救包回血，效果进核心结算可测，成本递增）+ 界面减文字（HUD 图标化）。**
...
- 已完成：v0.1~v0.4 全部功能 + **v0.5 可放置道具系统 + 界面减文字**
  + 战场 4 插槽 × 4 类道具（路障/尖刺/招财灯/急救包，效果进核心结算可测，成本递增）
  + 196 断言全绿（新增 27 条道具/hitTest-slot 测试）+ HUD 图标化减文字
  + Web/小程序同架构接入（hitTest slot 分支 + 点槽弹放置/拆除菜单）+ 核心数值零改动
  + Web 试玩版线上可玩（Camofox 实测：放置/屏障吸伤/招财灯产币/尖刺DPS/拆除/HUD减文字 全部验证）+ 文档同步更新，已推送 GitHub。
- 卡点：**等用户提供真实小程序 appid + 流量主 10 个广告位 ID**
  （需用户注册微信开发者账号/开通流量主操作，AI 无法代劳）
- 下一个 AI 接手第一步：问用户要 appid 和 adUnitId → 填 config.js
  → 开发者工具验证（重点：三屏切换 / 点触菜单+点槽位放道具在真机的点击命中与定位 / canvas 战场性能）→ 提审
- 若用户说"继续开发"：优先做 **道具种类扩充**（减速雾/护盾/自爆 + 稀有度）
  或 **转生/Prestige 系统**（跨关卡长线成长；累计通关数/星级换"转生点"，全局乘数，清档不清乘数）
- **改动守则**：
  - 战场观感 → 只动 `js/battle/battleView.js`；点触命中 → `hitTest`（含 slot 分支）
  - 数值/曲线 → 只动 `js/core/gameCore.js` 的 CURVE；道具效果 → gameCore 结算（别在渲染层模拟）
  - 关卡配置 → 只动 `js/levels.js`；道具定义 → gameCore 的 `ITEMS`（内聚，别拆独立文件，bootstrap 不 resolve require）
  - 三屏 UI 逻辑 → `web/game.js`（Web）+ `pages/index/index.js`（小程序），两端保持同架构
  - 改完必跑 `node tests/simulate.js`（196 全绿）
