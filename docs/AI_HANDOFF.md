...
**v0.6 新增：微信小程序手机端适配——触控热区放大 1.85×、自定义导航栏+安全区、canvas 保比例+dpr封顶2、底部图标状态坞（含游戏内快捷奖励广告位 game_bonus）、切后台省电、震动反馈、好友/朋友圈分享。Web 端零影响。**
下一步是：填真实广告位 ID + appid → 开发者工具真机预览 → 提审上线。详见 §6 待办清单。
...
- 已完成：v0.1~v0.5 全部功能 + **v0.6 微信小程序手机端适配**
  ...
  + 自定义导航栏 + 状态栏/底部安全区 + canvas 按屏宽保比例 + dpr 封顶 2
...
- **改动守则**：
  - 战场观感 → 只动 `js/battle/battleView.js`；点触命中 → `hitTest`（触控热区系数在 hitTest 内 `k`）
  ...
  - 改完必跑 `node tests/simulate.js`（200 全绿）
