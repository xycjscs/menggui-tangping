...
- v0.6.1 — **首页锁定一屏，永不滚动**（响应"首页不要有显示不全上下滚动的页面"）：
  - 首页容器锁死 `100dvh`（小程序 `100vh - 状态栏`）+ `overflow:hidden`，页面物理上无法产生滚动条
  - 关卡区 `flex:1` 吸收剩余高度：高屏行距自动拉开、矮屏（SE 375×667）自动压缩，8 关全部可见
  - 关卡卡改紧凑 3 行（关号+星级同行 / 名称 / 副标题，锁定时副标题显示解锁提示），单行 ellipsis 防裁切
  - Web 端实测 375×667 / 390×844 / 1280×800 三种视口：`overflow=0`、卡片零裁切（`scrollHeight≤clientHeight`）
  - 进入页/游戏屏不受影响（进入页内容量适中仍可滚动兜底；游戏屏 canvas 自适应）
